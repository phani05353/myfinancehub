...
// Build the monthly report as Slack Block Kit, directly from the structured
// data (NOT by scraping the rendered email HTML). The email is a nest of
// inline-styled layout tables that a generic html->mrkdwn converter collapses
// into an unreadable wall on mobile; assembling blocks from `d` gives a clean,
// skimmable card: a header, a net-result line, a 3-up fields grid, then one
// short section per breakdown (categories / merchants / biggest / budgets).
function monthlyReportSlackBlocks(d) {
  const netSign = d.net >= 0 ? '+' : '−';
  const blocks = [
    { type: 'header', text: { type: 'plain_text', text: `💰 ${d.monthLabel} Report`.slice(0, 150), emoji: true } },
    { type: 'context', elements: [{ type: 'mrkdwn', text: `💰 MyFinanceHub · ${d.txCount} transactions` }] },
    { type: 'divider' }
  ];

  // Net result + savings rate + month-over-month trend.
  let net = `*Net for ${d.monthLabel}*\n*${netSign}${money(d.net)}*`;
  if (d.savingsRate != null) {
    net += d.savingsRate >= 0 ? `  ·  ${d.savingsRate.toFixed(0)}% savings rate` : '  ·  spent more than earned';
  }
  if (d.spentDeltaPct != null) {
    const up = d.spentDeltaPct > 0;
    net += `\n${up ? '▲' : '▼'} ${Math.abs(d.spentDeltaPct).toFixed(0)}% spending vs last month`;
  }
  blocks.push({ type: 'section', text: { type: 'mrkdwn', text: net } });

  // Income / Spent / Avg-per-day as a fields grid (Slack lays these out in two
  // columns and reflows to one on a narrow phone — the mobile-friendly bit).
  blocks.push({
    type: 'section',
    fields: [
  ...
  addSection('Top spending categories',
    (d.topCategories || []).map((c) => `• ${esc(c.cat)} — ${money(c.total)}  ·  ${Math.round(c.pct)}%`));

  addSection('Top merchants',
    (d.topMerchants || []).map((m) => `• ${esc(m.payee)} _(${m.cnt}×)_ — ${money(m.total)}`));

  addSection('Biggest expenses',
    (d.biggestExpenses || []).map((t) => {
      const meta = [t.date, t.category].filter(Boolean).map(esc).join(' · ');
      return `• ${esc(t.payee)} — ${money(t.amount)}${meta ? `  _(${meta})_` : ''}`;
    }));

  addSection('Budget performance',
    (d.budgets || []).map((b) => {
      const dot = b.pct >= 100 ? '🔴' : b.pct >= 80 ? '🟡' : '🟢';
      return `${dot} ${esc(b.category)} — ${money(b.spent)} of ${money0(b.budget)} _(${b.pct.toFixed(0)}%)_`;
    }));

  blocks.push({
    type: 'context',
    elements: [{ type: 'mrkdwn', text: `Automated month-end summary · Generated ${esc(d.today)}` }]
  });
  return blocks;
}

module.exports = { monthlyReportHtml, monthlyReportSubject, monthlyReportSlackBlocks };
