...
| `LLM_BASE_URL` | `http://192.168.50.156:8000/v1` | Base URL of the local [oMLX](https://github.com/jundot/omlx) server, shared by receipt extraction and **Ask**. OpenAI-compatible, so it must end in `/v1`. Use the homelab host IP so the container can reach it. |
